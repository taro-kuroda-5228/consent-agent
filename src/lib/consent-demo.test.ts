...
describe("family-friendly rewriting stays faithful to selected sources (良い塩梅)", () => {
  ...
  it("creates long guideline/upload cards as physician review candidates without TOC/reference noise as the visible finding", () => {
    const guideline = createPhysicianUploadedEvidence({
      title: "JCS 2020 大動脈解離ガイドライン",
      fileName: "JCS2020_Ogino.pdf",
      extractedText:
        "目次 1 2 3 文献 References PMID 12345 著者一覧 山田 太郎 佐藤 花子 鈴木 一郎 田中 次郎 --- 急性A型大動脈解離では緊急手術を含む迅速な治療判断が重要であり、死亡、脳梗塞、腎不全、出血などの重篤な合併症を説明する。",
    });

    expect(guideline.physicianReviewTierLabel).toBe("採用候補");
    expect(guideline.clinicianSummary).toContain("ノイズ除去");
    expect(guideline.keyFindings?.[0]).toContain("急性A型大動脈解離");
    expect(guideline.keyFindings?.join(" ")).not.toMatch(/References|PMID|著者一覧|目次/);
    expect(guideline.displayForFamily).not.toMatch(/References|PMID|著者一覧|目次/);
  });

  it("marks upload cards with only bibliography/table-of-contents noise as exclude-recommended", () => {
    const noisy = createPhysicianUploadedEvidence({
      title: "長いPDF抽出ノイズ",
      fileName: "noisy-guideline.pdf",
      extractedText: "目次 1 2 3 4 文献 References PMID 12345 PMID 67890 著者一覧 山田 太郎 佐藤 花子 鈴木 一郎 田中 次郎 表 1 表 2",
    });

    expect(noisy.physicianReviewTierLabel).toBe("除外推奨");
    expect(noisy.physicianReviewReason).toContain("本文根拠として使える記載が不足");
    expect(noisy.keyFindings?.join(" ")).not.toMatch(/PMID 12345|山田 太郎/);
  });
});
